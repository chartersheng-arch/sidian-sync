...
def main() -> int:
    ...
    # Priority: --api-key CLI arg > TAVILY_API_KEY env > secrets file
    api_key = args.api_key or os.environ.get("TAVILY_API_KEY") or load_api_key()
    if not api_key:
        print(
            "Error: Tavily API key not found. Provide it via --api-key, "
            "TAVILY_API_KEY env var, or ../.secrets/tavily.key",
            file=sys.stderr,
        )
        return 1
