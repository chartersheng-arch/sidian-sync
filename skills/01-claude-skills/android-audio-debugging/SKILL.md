...
flowchart TB
    subgraph INPUT["输入层"]
        LOG[(logcat/dmesg<br/>音频日志)]
        TRACE[(perfetto trace<br/>时序跟踪)]
        CODE[(HAL/Framework<br/>源码)]
        BUG_TXT[(BUG.txt<br/>问题描述)]
        CASES[(历史案例<br/>对比)]
    end

    subgraph SKILL["SKILL 主导层"]
        direction TB
        S0[阶段0<br/>信息收集] --> S1[阶段1<br/>自动分析]
        S1 --> S2[阶段2<br/>疑点验证]
        S2 --> S3[阶段3<br/>多轮推理]
        S3 --> S4[阶段4<br/>报告输出]

        S0 --> BUG[BUGrecord<br/>自动记录]
        S3 --> BUG
        BUG --> POOL[feedback_pool<br/>待归纳]

        S0 --> USER_DB[用户自定义<br/>知识库]
        S1 --> USER_DB
    end

    subgraph MCP["MCP 辅助层"]
        ALP[audio_log_parser<br/>日志解析]
        CL[code_locator<br/>源码定位]
        DC[device_collector<br/>设备取证]
        CM[case_matcher<br/>案例匹配]
        C7[context7<br/>官方文档]
        PA[perfetto-audio<br/>Trace分析]
        OC[obsidian-charter<br/>案例保存]
    end
    ...
    SKILL --> REPORT[诊断报告<br/>根因+建议]
    SKILL --> KNOWLEDGE[(知识库<br/>沉淀积累)]
```
...
| 层级 | 组件 | 职责 |
|------|------|------|
| **输入层** | BUG.txt | 优先解析的问题描述文件 |
| | 日志/trace/源码/案例 | 原始分析数据 |
| **SKILL主导层** | 阶段0-4 | 四阶段调试方法论 |
| | BUGrecord | 用户侧分析偏离记录 |
| | feedback_pool | 待归纳的BUGrecord集合 |
| | 用户知识库 | 案例匹配与参考 |
| **MCP辅助层** | audio_log_parser | 日志分层解析 |
| | code_locator | 源码定位与调用链 |
| | device_collector | adb命令与dumpsys |
| | case_matcher | 历史案例匹配 |
| | context7 | 官方文档查证 |
| | perfetto-audio | Trace分析 |
| | obsidian-charter | 案例保存到知识库 |
| **输出层** | 诊断报告 | 带凭证的根因分析 |
| | 知识库 | 问题沉淀与复用 |
...
├── SKILL.md                    # 主技能文件
├── user_config.md              # 用户自定义知识库配置
├── BUGrecord.md                # 用户侧偏离记录模板
├── mcp_server/
│   └── README.md               # MCP服务器配置说明
├── references/
│   ├── framework-layers.md     # Audio框架分层知识
│   ├── hal-interface.md        # HAL接口与状态机
│   ├── failure-patterns.md     # 常见故障模式库
│   ├── log-analysis.md         # 日志分析技巧
│   ├── case-library.md         # 历史案例库格式
│   ├── feedback-synthesis.md   # BUGrecord归纳方法论
│   └── feedback-loop-guide.md  # 反馈收集流程指南
└── feedback_pool/              # 待归纳的BUGrecord集合
    ├── BUGrecord1.md
    ├── BUGrecord2.md
    └── ...
...
**⚠️ 重要：当用户提供目录时，必须优先检查是否存在 BUG.txt**
...
- 问题现象: <描述>
- 切入点: <用户怀疑的方向>
- 相关文件路径: <log/trace/code中的关键文件>
```

**处理流程:**

```
1. 【优先】检查目录中是否存在 BUG.txt
   ↓
2. 若存在 BUG.txt，优先解析其内容：
   ├── 提取问题现象描述
   ├── 提取切入点/疑点方向
   ├── 提取相关文件路径（log/trace/code）
   └── 验证这些文件是否存在于目录中
   ↓
3. 基于 BUG.txt 的信息确定分析优先级：
   ├── 优先分析 BUG.txt 中指明的切入点
   ├── 优先检查 BUG.txt 中提到的相关文件
   └── 日志范围以 BUG.txt 描述的时间段为准
   ↓
4. 若 BUG.txt 中提到的文件不存在，询问用户补充
```
...
相关文件路径:
  - log/audio_log.txt
  - trace/perfetto_trace.perfetto-trace
  - code/AudioPolicyManager.cpp
```
...
现象: 通话时对方听不到声音
切入点: SCO 路由问题（用户提供）
优先分析: AudioPolicyManager 路由逻辑
优先日志: log/audio_log.txt
优先 trace: trace/perfetto_trace.perfetto-trace
相关代码: code/AudioPolicyManager.cpp
```
...
1. 扫描目录结构，列出所有可用文件
2. 按以下优先级自动判断分析入口:
   ├── 若有 perfetto trace → perfetto-audio 分析
   ├── 若有 audio dump → PCM 数据分析
   ├── 若有 logcat/dmesg → 日志分层解析
   └── 若有源码 → 代码逻辑分析
3. 询问用户确认分析方向
```
...
**bugreport结构**: bugreport可能采用多层压缩格式，需要多次解压才能获取最终文本log。
...
**适用场景**: 当用户提供bugreport-*.zip文件时使用此方法解压

### 0.7 信息提取模板

**基于 BUG.txt 的优先提取（当存在 BUG.txt 时）:**

```
【问题提取 - 来自 BUG.txt】
现象: <BUG.txt 中描述的问题现象>
切入点: <BUG.txt 中用户提供的怀疑方向>
优先日志: <BUG.txt 中提到的 log 文件路径>
优先 trace: <BUG.txt 中提到的 trace 文件路径>
相关代码: <BUG.txt 中提到的代码文件路径>
平台验证: <根据提到的文件路径判断平台/供应商>
```

**标准信息提取（无 BUG.txt 或补充信息时）:**
...
1. ✅ **用户明确指令**：当用户说"生成BUGrecord"、"启用BUGrecord"、"记录全程"、"记录此问题"等时，**无条件触发**
2. ✅ 已分析出至少一个根因（置信度中/高）
3. ❌ 用户没有采纳/纠正第一个根因，包括：
   - 用户明确说"继续分析其他方向"
   - 用户说"根因不对"、"不是这个"
   - 用户追问但未确认根因正确
   - 分析轮次 ≥ 3 轮但未收敛
4. ❗ **排除"设计如此"**：如果最终确认问题是代码逻辑设计如此（而非bug），则不记录
...
| `BUGrecord.md` | 用户侧偏离记录模板 |
| `mcp_server/README.md` | MCP服务器配置说明 |
