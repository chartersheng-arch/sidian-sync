...
| 操作类型 | 触发条件 | 同步动作 |
|---------|---------|---------|
| 创建新 skill | 在 `C:\Users\nijiasheng1\.claude\skills\` 下新增 .md 文件 | 将新 skill 同步到 `D:\sidian-charter\claude skills\` |
| 修改 skill | 已有 skill 文件内容发生变更 | 用本地版本覆盖 Obsidian 版本 |
...
2. 执行同步：
   - 创建/修改：读取本地 skill 文件 → 写入/覆盖到 D:\sidian-charter\claude skills\{skill-name}.md
