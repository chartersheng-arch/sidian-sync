...
```
1. 同步目录
   cp -r "C:/Users/nijiasheng1/.claude/skills/" "D:/sidian-charter/skills/01-claude skills/"

2. 检查文件名一致性
   - 索引链接格式: [[01-claude skills/{skill-name}/SKILL.md|...]]
   - 如果本地是 skill.md 但索引期望 SKILL.md，需要重命名

3. 验证链接功能
   ls "D:/sidian-charter/skills/01-claude skills/{skill-name}/" | grep -i skill
```
...
3. 更新索引：
   - 同步完成后，重新生成 00 - Skills Index.md 索引文件
   - 确保索引包含所有同步后的 skills
